...
public class OperatorServiceImplTest {
  ...
  void addOperator() {
    ...
    operator.setFname("operateur2");
    operator.setLname("martin2");
    operator.setPassword("pwd1525");
    this.operatorService.addOperator(operator);
    assertEquals(this.operatorService.retrieveAllOperators().size(),2);
    assertEquals(this.operatorService.retrieveOperator(2L).getPassword(),"pwd1525");
  }
  ...
  void retrieveOperator() {
    final Operator operator = this.operatorService.retrieveOperator(1L);
    assertEquals("martin", operator.getLname());
    assertEquals("operateur1", operator.getFname());
    assertEquals("pwd15", operator.getPassword());
  }
